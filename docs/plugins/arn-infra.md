...
Skill names below are host-neutral. In Claude Code, invoke Infra skills as slash commands, for example `/arn-infra-wizard`; in Codex, install with `codex plugin add arn-infra@arn-marketplace`, then prompt with the same skill name in plain language, for example `codex "arn-infra-wizard"`.
