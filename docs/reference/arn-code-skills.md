...
Skill names are shown without host-specific syntax. In Claude Code, invoke a skill as `/skill-name`; in Codex, prompt with the same name, for example `codex "arn-planning add rate limiting"`.
...
| `arn-code-help` | Detect current position in the Arness Code development pipeline, render a diagram with the active stage marked, and suggest the next skill. Cross-plugin aware — detects Spark and Infra activity and provides hints. Read-only. | 1.2.0 |
