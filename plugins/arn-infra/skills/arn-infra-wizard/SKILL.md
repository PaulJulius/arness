...
Walk the user through the entire Arness infrastructure pipeline in a single continuous session. The wizard invokes each pipeline skill in sequence using the Codex skill invocation, only pausing at genuine decision gates where user input is needed.
...
> Read `<arn-infra-plugin-root>/skills/arn-infra-wizard/references/entry-modes.md` for the complete entry mode detection rules, conditions, and pipeline sequences.
...
> Read <arn-infra-plugin-root>/skills/arn-infra-wizard/references/full-pipeline-flow.md
...
2. Invoke Codex skill `arn-infra-change-spec` with the existing artifacts as input context:
...
> Read <arn-infra-plugin-root>/skills/arn-infra-wizard/references/decision-gates.md

---

## Artifact Detection (Resumability)

> Read <arn-infra-plugin-root>/skills/arn-infra-wizard/references/artifact-detection.md
...
> Codex skill `arn-infra-discover`
...
If **Yes:**
> Codex skill `arn-infra-containerize`
...
- **Deploy to staging/production:** Codex skill `arn-infra-deploy` (pass the target environment as context)
- **Dry run only:** Codex skill `arn-infra-deploy` (pass dry-run mode as context)
...
- **Verify then CI/CD:**
  > Codex skill `arn-infra-verify`
  > Then: Codex skill `arn-infra-pipeline`

- **Verify only:**
  > Codex skill `arn-infra-verify`

- **CI/CD only:**
  > Codex skill `arn-infra-pipeline`
...
- **Both:**
  > Codex skill `arn-infra-secrets`
  > Then: Codex skill `arn-infra-monitor`

- **Secrets only:**
  > Codex skill `arn-infra-secrets`

- **Monitoring only:**
  > Codex skill `arn-infra-monitor`
...
- **Sub-skill fails or errors out:** Present the error to the user. Ask the user: "The [skill-name] step encountered an error: [error summary]. What would you like to do?" Options: retry this step / skip this step / abort the wizard. If retry, re-invoke the same skill. If skip, continue to the next gate. If abort, show what was completed and exit.
...
- All pipeline work is done by the invoked skills via the Codex skill invocation.
- Sub-skill invocations use the Codex skill invocation with fully qualified names: Codex skill `arn-infra-<name>`.
